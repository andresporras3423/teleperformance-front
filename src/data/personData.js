...
export const updatePerson = async (person_id, identity_type_id,
  ...
  const response = await fetch('https://teleperformance-back-api.azurewebsites.net/api/Person', {
    ...
    body: JSON.stringify({
      person_id, identity_type_id, identity_number, company_name, first_name, second_name, first_last_name, second_last_name, email, allow_email_message, allow_phone_message,
    }),
  });
  const dataResponse = await response.json();
  return dataResponse;
